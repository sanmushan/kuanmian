package com.benxiang.noodles.view.swingindicator;
...
public class SwingIndicator extends View{
	...
	private void drawStatic(Canvas canvas){
		...
		for(int i = 0; i<5; i++){
			...
			oval.top = top;
			if (i<2){
				paint.setColor(0xFF1C86EE);

				;
			} else if (i == 2) {
				Shader mShader = new LinearGradient(oval.left, oval.top, oval.left + diameter , oval.top + diameter,

						new int[] { 0xFF1C86EE, 0xFFEE7621,  Color.YELLOW },

						null, Shader.TileMode.REPEAT);
				paint.setShader(mShader);
			}else {
				paint.setShader(null);
				paint.setColor(0xFFEE7621);
			}
